const number = ""; //填入邀请码或者通过$argumeent传入
...
class Pikpak {
  constructor(pikpak_id, invitation_Code) {
    ...
    this.captcha_token =
      $persistentStore.read("pikpak_captcha_token") || pikpak_id[2];
    this.body = {
...
(async () => {
  if (typeof $request === "object") {
    ...
    client_id &&
      ...
      $persistentStore.write(
        `${client_id},${device_id},${captcha_token}`,
        "pikpak_id"
      ) &&
  ...
  const invitation_Code =
    (typeof $argument === "string" && Number($argument)) || Number(number) || 0;
